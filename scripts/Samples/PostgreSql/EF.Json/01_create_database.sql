-- =============================================================================
-- AuditaX Sample: Entity Framework + PostgreSQL + JSON
-- Script 01: Create Database
-- =============================================================================
-- Database: auditax_ef_postgresql_json
-- =============================================================================

-- Connect to postgres database first, then run this script
-- psql -h localhost -U postgres -d postgres -f 01_create_database.sql

-- Terminate existing connections
SELECT pg_terminate_backend(pg_stat_activity.pid)
FROM pg_stat_activity
WHERE pg_stat_activity.datname = 'auditax_ef_postgresql_json'
  AND pid <> pg_backend_pid();

-- Drop database if exists (for clean testing)
DROP DATABASE IF EXISTS auditax_ef_postgresql_json;

-- Create database
CREATE DATABASE auditax_ef_postgresql_json;

-- Success message
DO $$
BEGIN
    RAISE NOTICE 'Database auditax_ef_postgresql_json created successfully.';
END $$;
